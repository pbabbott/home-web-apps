...
export const DotGridBackground = ({
  ...
  const blackPercent = active ? '5%' : '40%';

  return (
    <div
      className={`absolute inset-0 -z-10 ${colorClass} transition-all duration-300 cursor-pointer`}
      style={{
        backgroundImage: `radial-gradient(currentColor ${dotSize}px, transparent ${dotSize}px)`,
        backgroundSize: `${spacing}px ${spacing}px`,
        maskImage: `radial-gradient(circle at center, black 0%, black ${blackPercent}, transparent 100%)`,
        WebkitMaskImage: `radial-gradient(circle at center, black 0%, black ${blackPercent}, transparent 100%)`,
      }}
    />
  );
};
