...
| `security-scan-report.sh`      | Scans the locally-tagged blog/diagram-maker/fui-components images with Trivy and writes a combined Markdown vulnerability report. | Run by `security-scan` after the pull step; the report is posted to the job summary and uploaded as an artifact, and its exit code gates the check on fixable CRITICAL/HIGH findings. |
